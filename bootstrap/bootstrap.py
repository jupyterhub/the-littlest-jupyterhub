...
Constraints:
    ...
    - The entire script should be compatible with Python 3.10, which is the default on
      Ubuntu 22.04.
...
def ensure_host_system_can_install_tljh():
    ...
    """
    # Require Ubuntu 22.04+ or Debian 12+
    ...
    if distro not in ["ubuntu", "debian"]:
        ...
        sys.exit(1)
    elif distro == "debian" and _parse_version(version) < (12,):
        print("The Littlest JupyterHub requires Debian 12 or higher")
        sys.exit(1)

    # Require Python 3.10+
    if sys.version_info < (3, 10):
        print(
            f"bootstrap.py must be run with at least Python 3.10, found {sys.version}"
        )
        sys.exit(1)
